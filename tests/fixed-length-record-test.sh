...
start_python_test 'fixed-length-record.py'
...
function runonetest () {
  ...
  cat /tmp/fixed1 | sed -E "s/(${FNAME}\s+)(${LNAME}\s+)[0-9]{8}/\1\2${BDAY}/g" > /tmp/e1
...
runonetest "${TEXTFILEPATH}/fixed-length.txt" Adam Smithers 00000000
runonetest "$TEXTFILEPATH/poem.txt" Adam Smith 11111111
runonetest "${TEXTFILEPATH}/fixed-length.txt" Adam Smith 11111900
